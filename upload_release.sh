...
if [ ! -f "release/Routini-Setup-0.1.3-pwa.exe" ]; then
    echo "❌ Executável Windows não encontrado. Execute: npm run electron:build:win"
    exit 1
fi

if [ ! -f "release/Routini-0.1.3-pwa-portable.exe" ]; then
...
# Windows Setup
gh release upload v0.1.3-pwa release/Routini-Setup-0.1.3-pwa.exe \
    --repo elissoncardoso1/routini \
    --clobber

# Windows Portable
gh release upload v0.1.3-pwa release/Routini-0.1.3-pwa-portable.exe \
    --repo elissoncardoso1/routini \
    --clobber

# macOS DMG (se existir)
if [ -f "release/Routini-0.1.3-pwa-arm64.dmg" ]; then
    gh release upload v0.1.3-pwa release/Routini-0.1.3-pwa-arm64.dmg \
        --repo elissoncardoso1/routini \
        --clobber
fi

# macOS ZIP (se existir)
if [ -f "release/Routini-0.1.3-pwa-arm64-mac.zip" ]; then
    gh release upload v0.1.3-pwa release/Routini-0.1.3-pwa-arm64-mac.zip \
        --repo elissoncardoso1/routini \
        --clobber
fi

echo "✅ Upload concluído!"
echo "🌐 Acesse: https://github.com/elissoncardoso1/routini/releases/tag/v0.1.3-pwa" 